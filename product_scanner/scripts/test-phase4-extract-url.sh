#!/bin/bash
# Phase 4 URL 기반 상품 추출 워크플로우 테스트
#
# Phase 4 TypedNodeStrategy 기반 extract_url 노드 테스트
# - PlatformScannerRegistry를 통한 플랫폼별 스캔
# - Supabase 조회 없음 (db: null, comparison: null)
#
# 사용법:
#   ./test-phase4-extract-url.sh "https://www.oliveyoung.co.kr/store/goods/getGoodsDetail.do?goodsNo=A000000223456"
#   URL="https://..." ./test-phase4-extract-url.sh
...
if [ -z "$URL" ]; then
  ...
  echo ""
  echo "사용법:"
  echo "  ./test-phase4-extract-url.sh \"https://www.oliveyoung.co.kr/...\""
  echo "  URL=\"https://...\" ./test-phase4-extract-url.sh"
  echo ""
  echo "지원 플랫폼:"
...
JSON_PAYLOAD=$(cat <<EOF
{
  ...
  "metadata": {
    "test": true,
    "phase": "4",
    "description": "Phase 4 URL 기반 상품 추출 테스트"
  }
}
EOF
)
