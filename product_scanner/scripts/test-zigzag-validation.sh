...
# 지그재그 워크플로우 검증 스크립트
#
# 타입드 노드 기반 검증 파이프라인 테스트 (GraphQL API 방식)
...
{
  "workflow_id": "zigzag-validation-v2",
  "priority": 5,
  "params": {
    "platform": "zigzag",
  ...
  "metadata": {
    ...
    "description": "지그재그 GraphQL API 기반 검증 테스트 (Gentle Rate Limiting)"
  }
}
EOF
)
