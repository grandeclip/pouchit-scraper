#!/bin/bash
# Phase 4 Product UUID 기반 멀티 플랫폼 상품 추출 워크플로우 테스트
#
# Phase 4 TypedNodeStrategy 기반 extract_product 노드 테스트
# - Product ID (UUID)로 모든 product_set 조회
# - PlatformScannerRegistry를 통한 플랫폼별 스캔
# - DB 데이터와 스캔 결과 비교
#
# 사용법:
#   ./test-phase4-extract-product.sh "product-uuid"
#   PRODUCT_ID="uuid" SALE_STATUS="on_sale" ./test-phase4-extract-product.sh
...
echo -e "${CYAN}🧪 Phase 4 Product UUID 기반 멀티 플랫폼 상품 추출 테스트${NC}"
echo -e "${BLUE}📊 설정:${NC}"
  ...
  JSON_PAYLOAD=$(cat <<EOF
{
  ...
  "metadata": {
    "test": true,
    "phase": "4",
    "description": "Phase 4 Product UUID 기반 멀티 플랫폼 추출 테스트"
  }
}
EOF
)
