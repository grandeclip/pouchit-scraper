#!/bin/bash
# Phase 4 지그재그 워크플로우 검증 스크립트
#
# Phase 4 타입드 노드 기반 검증 파이프라인 테스트 (GraphQL API 방식)
# - FetchProductNode → ScanProductNode (GraphQL) → ValidateProductNode
#   → CompareProductNode → SaveResultNode → NotifyResultNode
#
# 주의: zigzag는 GraphQL API 기반이므로 gentle rate limiting 적용
# - concurrency: 1 (순차 처리)
...
echo "🧪 Phase 4 지그재그 워크플로우 테스트 시작"
echo "📊 설정:"
echo "   - LIMIT=${LIMIT}"
...
echo ""
echo "⚠️  지그재그는 GraphQL API 기반이므로 gentle rate limiting 적용됨"
echo ""
...
{
  "workflow_id": "phase4-zigzag-validation-v2",
  "priority": 5,
  "params": {
    "platform": "zigzag",
  ...
  "metadata": {
    "test": true,
    "phase": "4",
    "description": "Phase 4 지그재그 GraphQL API 기반 검증 테스트 (Gentle Rate Limiting)"
  }
}
EOF
)
